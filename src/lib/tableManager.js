  ...
  const MS_PER_DAY = 86400000;
    ...
    const fillAction = async (number) => {
        ...
        for (
            ...
            transactions.add({ date: new Date(d), description, value, actual: false, domain });
        }
        buildTable();
        transactions.save();
    };
    ...
    const buildTable = () => {
        ...
        rows.length = 0;
        transactions.sort();
        const periodStartDate = transactions.data[0] ? new Date(transactions.data[0].date) : new Date();
        ...
        const summary = menu.addText(
            ...
            `${dateFormatter.string(now)} - ${daysBetween(
                now,
                endDate
            )} days left`
        );
